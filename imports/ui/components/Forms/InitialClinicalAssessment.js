...
import SummaryICA from "./SummaryICA";
import AISS from "./AISS";
import Charleston from "./Charleston";
import Glasgow from "/ICA/Glasgow";
...
class InitialClinicalAssessment extends Component {
  ...
  async shouldComponentUpdate(nextProps, nextState) {
    ...
    return this.state !== nextState || this.props.data.ini !== nextProps;
  }
  ...
  handleSubmit = async (initialClinicalAssessment, history, match) => {
    ...
    const newIca = _.omit(initialClinicalAssessment, ["__typename"]);
  ...
  render() {
    ...
    return loading ? (
      ...
      >
        {LoadablePart ? (
          <LoadablePart model={this.state.model} changeDate={this.changeDate} />
        ) : (
          ""
        )}
        <WizardFooter
        ...
        refetchQueries: [
          {
            ...
            variables: { patientId: questionnaire.patientId }
          }
        ]
      };
    }
